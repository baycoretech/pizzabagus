<?php
    ...
    if(isset($_POST['blogid'])) {
		$blogid = abs($_POST['blogid']);
		$ip     = get_real_ip();

		$class_blog     = new Blog($pdo);
		$check_ip_likes = $class_blog->check_ip_likes($blogid, $ip);
		...
		if($check_ip_likes == 0) {
			...
			$add_likes = $class_blog->add_likes($blogid, $ip, $now_date);
			$count_likes = $class_blog->count_likes($blogid);
