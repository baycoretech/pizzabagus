    ...
    if(isset($_SESSION['order'])) {
		...
		if($total == 0) {
			...
			if(!isset($lang)) {
        		...
        		$request_text = 'NOTES AND SPECIAL REQUEST';
                $price_table = 'PRICE';
                $qty_table   = 'QUANTITY';
                ...
                if($lang == $default_language->cr_languageCode) {
        			...
        			$request_text = 'CATATAN DAN PERMINTAAN KHUSUS';
                    $price_table = 'HARGA';
	                $qty_table   = 'JUMLAH';
	                ...
	                //Alert
	                $alert_failed_text = 'Gagal. Tidak bisa mengulas pesanan anda. Silahkan coba lagi.'; 
	                $alert_error_text  = 'Kesalahan. Tidak bisa mengulas pesanan anda. Silahkan coba lagi.'; 
                }
            }
?>
<div class="container">
	<div class="row">
		...
		<div class="col-md-6">
			...
			<dl class="dl-horizontal">
			  	...
			  	<dd><span>:</span> Cash</dd>
			</dl>
		</div>
		<div class="col-md-6">
			<h4 class="m-b-20"><?php echo $request_text ?></h4>
			<div class="form-group">
				<textarea id="special-request" class="form-control" name="request" rows="5"><?php echo $_SESSION['request'] ?></textarea>
			</div>
		</div>
		<div class="col-md-12">
			<hr>
			<button id="button-next-step" class="btn btn-group btn-pizzabagus pull-right"><?php echo $next_btn ?></button>
			<a href="<?php echo $router->generate('specific-page-lang', array('lang' => $lang, 'page' => 'my-order')) ?>" class="btn btn-group btn-pizzabagus pull-right m-r-10"><?php echo $back_btn ?></a>
		</div>
	</div>
</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#button-next-step').click(function() {
			var action      = 'review';
			var request     = $('#special-request').val();
			var dataString  = 'act='+action+'&request='+request;
			$.ajax({
	            type: "POST",
	            url:  "<?php echo MURL ?>cr-include/ajax/tc-cart.php",
	            data: dataString,
	            cache: false,
	            	beforeSend: function(){ $("#button-next-step").html('<i class="fa fa-spinner fa-pulse"></i>');$("#button-next-step").attr('disabled','disabled');},
	            	success: function(data){
		            	if(data == "success") {
		            		setTimeout(function() {
	                            window.location="<?php echo $router->generate('specific-page-lang', array('lang' => $lang, 'page' => 'checkout-review')); ?>";
	                        }, 2000);
		            	}
		            	else if(data == "failed") {
		            		$("#button-next-step").removeAttr('disabled');
		            		$("#button-next-step").html('<?php echo $next_btn ?>');
