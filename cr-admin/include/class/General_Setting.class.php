...
class General_Setting {
	...
	public function save_visitor($visitor_ip, $now_date) {
        $check_ip  = $this->pdo->query("SELECT * FROM cr_visitor WHERE cr_visitorIP = '$visitor_ip'");
        $insert_ip = $this->pdo->prepare("INSERT INTO cr_visitor (cr_visitorIP, cr_visitorDate) VALUES(?, ?)");
    ...
    public function total_created_page() {
        $check_menu    = $this->pdo->query("SELECT * FROM cr_menu WHERE cr_menuHasSub <> '1' AND cr_option <> 'customlink'");
        $total_menu    = $check_menu->rowCount();
        $check_submenu = $this->pdo->query("SELECT * FROM cr_submenu WHERE cr_option <> 'customlink'");
