    ...
    <div class="col-md-8">
        <div class="panel panel-inverse">
            ...
            <div class="panel-body">
                <form id="form-add-service" data-parsley-validate action="" method="POST">
                    <input id="mediafile" type="hidden" name="photo" value="">
                    <div class="form-group">
                        <label class="control-label">Service Name</label>
                        <input class="form-control" placeholder="Service Name" type="text" name="name" data-parsley-maxlength="100" required>
                    </div>
                    <div class="form-group">
                        <label class="control-label">Description</label>
                        <textarea name="desc" class="form-control" rows="10" placeholder="Service Description" required></textarea>
                    </div>
            </div>
            <div class="panel-footer">
                    <button id="button-add-service" type="submit" class="btn btn-success m-r-5 m-b-5"><i class="fa fa-check"></i> Save</button>
    ...
    $(document).ready(function() {
        ...
        $("#form-add-service").submit(function(event){
            if ($(this).parsley().isValid()) {
                ...
                add_service = $.ajax({
                    url: "<?php echo MADMINURL ?>ajax/service-add.php",
                    type: "post",
                    beforeSend: function(){ $("#button-add-service").html('<i class="fa fa-spinner fa-pulse"></i> Saving...');$("#button-add-service").attr('disabled','disabled');},
                    data: serializedData
                });
                add_service.done(function (msg){
                    if(msg == 'empty-field') {
                        ...
                        $.gritter.add({
                            ...
                            text:"Can't add new service. It should have 100 characters or less. Please try again.",
                            image:"<?php echo MADMINURL.'assets/img/cr.png' ?>",
                            sticky:false,
                            time:""
                        });
                        ...
                        $("#button-add-service").html('<i class="fa fa-check"></i> Save');
                        $.gritter.add({
                            title:"Failed! Can't add new service",
                        ...
                        $("#button-add-service").html('<i class="fa fa-check"></i> Save');
                        $.gritter.add({
                            title:"Error! Can't add new service",
                            text:"Please try again.",
                            image:"<?php echo MADMINURL.'assets/img/cr.png' ?>",
                            sticky:false,
                            time:""
