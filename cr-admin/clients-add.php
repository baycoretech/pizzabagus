    ...
    <div class="col-md-8">
        <div class="panel panel-inverse">
            ...
            <div class="panel-body">
                <form id="form-add-client" data-parsley-validate action="" method="POST">
                    <input id="mediafile" type="hidden" name="photo" value="">
                    <div class="form-group">
                        <label class="control-label">Name</label>
                        <input class="form-control" placeholder="Name" type="text" name="name" data-parsley-maxlength="30" required>
                    </div>
                    <div class="form-group">
                        <label class="control-label">Link</label>
                        <input class="form-control" placeholder="Link" type="text" name="link" data-parsley-type="url">
                    </div>
            </div>
            <div class="panel-footer">
                    <button id="button-add-client" type="submit" class="btn btn-success m-r-5 m-b-5"><i class="fa fa-check"></i> Save</button>
    ...
    $(document).ready(function() {
        ...
        $("#form-add-client").submit(function(event){
            if ($(this).parsley().isValid()) {
                ...
                add_client = $.ajax({
                    url: "<?php echo MADMINURL ?>ajax/clients-add.php",
                    type: "post",
                    beforeSend: function(){ $("#button-add-client").html('<i class="fa fa-spinner fa-pulse"></i> Saving...');$("#button-add-client").attr('disabled','disabled');$(".button-cancel").attr('disabled','disabled')},
                    data: serializedData
                });
                add_client.done(function (msg){
                    if(msg == 'empty-field') {
                        $(".button-cancel").removeAttr('disabled');
                        $("#button-add-client").removeAttr('disabled');
                        $("#button-add-client").html('<i class="fa fa-check"></i> Save');
                        $.gritter.add({
                            title:"Failed! Please fill all required field",
                            text:"Can't add new client. Please try again.",
                        ...
                        $.gritter.add({
                            ...
                            text:"Can't add new client. It should have 50 characters or less. Please try again.",
                            image:"<?php echo MADMINURL.'assets/img/cr.png' ?>",
                            sticky:false,
                            time:""
                        });
                        ...
                        $.gritter.add({
                            ...
                            text:"Can't add new client. You have to upload the client logo or image. Please try again.",
                            image:"<?php echo MADMINURL.'assets/img/cr.png' ?>",
                            sticky:false,
                            time:""
                        });
                        ...
                        $("#button-add-client").html('<i class="fa fa-check"></i> Save');
                        $.gritter.add({
                            title:"Failed! Can't add new client",
                        ...
                        $("#button-add-client").html('<i class="fa fa-check"></i> Save');
                        $.gritter.add({
                            title:"Error! Can't add new client",
                            text:"Please try again.",
                            image:"<?php echo MADMINURL.'assets/img/cr.png' ?>",
                            sticky:false,
                            time:""
