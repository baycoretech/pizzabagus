<?php
    $function_edit_page_featured = $class_page->edit_page_featured($action);
    $featured_id    = $function_edit_page_featured->cr_featuredID;
    $featured_desc  = $function_edit_page_featured->cr_featuredDesc;
    $featured_image = $function_edit_page_featured->cr_featuredImage;
?>
<div class="row">
    <!-- begin col-8 -->
    <div class="col-md-8">
        ...
        <div class="panel panel-inverse">
            <div class="panel-heading">
                ...
                <h4 class="panel-title">Page Description</h4>
            </div>
                <div id="" class="panel-body">
                    <form id="form-edit-featured" data-parsley-validate action="" method="POST">
                        <input type="hidden" name="page_id" value="<?php echo $featured_id ?>">
                        <input type="hidden" name="link" value="<?php echo $action ?>">
                        <input id="mediafile" type="hidden" name="photo" value="<?php echo $featured_image ?>">
                        
                        <div class="form-group">
                            <label class="control-label">Description</label>
                            <textarea name="desc"><?php echo $featured_desc ?></textarea>
                        </div>
                </div>
            <div class="panel-footer">
                <button id="button-edit-featured" type="submit" class="btn btn-success m-r-5 m-b-5"><i class="fa fa-check"></i> Save</button>
                <button type="button" class="btn btn-white pull-right m-r-5 m-b-5" onclick="location.href='<?php echo $router->generate('admin-dashboard-action', array('section' => $section, 'action' => $action)) ?>'">Cancel</button>
                </form>
            </div>
         </div>
        <!-- end panel -->
    ...
    <div class="col-md-4">
        ...
        <div class="panel panel-inverse">
            ...
            <div class="panel-body">
                <div id="used-image-container"><img id="used-image" src="<?php echo MURL.'cr-editor/images/'.$featured_image ?>" width="100%"></div>
...
<script>
        $(document).ready(function() {
            ...
            var edit_featured;
            $("#form-edit-featured").submit(function(event){
                if ($(this).parsley().isValid()) {
                    ...
                    edit_featured = $.ajax({
                        url: "<?php echo MADMINURL ?>ajax/featured-update.php",
                        type: "post",
                        beforeSend: function(){ $("#button-edit-featured").html('<i class="fa fa-spinner fa-pulse"></i> Saving...');$("#button-edit-featured").attr('disabled','disabled');},
                        data: serializedData
                    });
                    edit_featured.done(function (msg){
                        if(msg == 'true') {
                            $.gritter.add({
                                title:"Success!",
                            ...
                            $("#button-edit-featured").removeAttr('disabled');
                            $("#button-edit-featured").html('<i class="fa fa-check"></i> Save');
                            $.gritter.add({
