<?php
    ...
    if(empty($name) || empty($link) || empty($family) || empty($applied)) {
		...
		elseif(!empty($family) && strlen($family)>100) {
			...
			$function_check_update_fonts = $class_appearance->check_update_fonts($applied, $font_idh);
			if($function_check_update_fonts < 1) {
				//Set timezone
		    	...
		    	$function_update_font = $class_appearance->update_font($name, $link, $family, $applied, $admin_login_id, $now_date, $font_idh);
		    	if($function_update_font == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
			}
