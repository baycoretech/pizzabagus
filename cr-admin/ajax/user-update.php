<?php
	...
	elseif(strlen($displayname) > 20) {
		...
		elseif(!empty($tw) && filter_var($tw, FILTER_VALIDATE_URL) === false) {
	    	...
	    	$function_update_administrator = $class_administrator->update_administrator($username, $password, $email, $photo, $displayname, $level, $aboutyou, $fb, $gp, $tw, $user_id, $admin_login_id, $now_date);
	    	if($function_update_administrator == true) {
    			echo 'true';
	    	}
	    	else 
	    		echo 'false';
