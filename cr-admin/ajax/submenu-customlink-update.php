<?php
	...
	elseif(filter_var($link, FILTER_VALIDATE_URL) === false) {
	    ...
	    $function_check_update_menu_title = $class_menu->check_update_menu_title($title, $submenu_idh);
		if($function_check_update_menu_title == true) {
			$function_update_subcustom_link = $class_menu->update_subcustom_link($title, $title_id, $link, $status, $option, $submenu_idh, $admin_login_id, $now_date);
			if($function_update_subcustom_link == true) {
			    echo "true";
			}
			else {
				echo "false";
