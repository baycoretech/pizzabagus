    ...
    if(empty($title) || empty($content) || empty($cat) || empty($status)) {
	    ...
	    elseif(strlen($metadesc) > 155) {
	        ...
	        if($function_check_blog_name_edit == true) {
		        ...
		        $function_update_blog_sound = $class_blog->update_blog_sound($title, $content, $link, $blog_type_id, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $blog_idh, $admin_login_id, $now_date);
		        if($function_update_blog_sound == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
		    }
