<?php
    ...
    $portfolio_idh   = $_POST['portfolio_idh'];
    	...
    	if($photo == MADMINURL."assets/img/no-pic-items.png") {
		    ...
		    elseif(strlen($slider_image) < 1) {
		    	echo "no-slider";
		      	...
		      	$function_check_update_name_portfolio = $class_portfolio->check_update_name_portfolio($title, $portfolio_idh);
		        if($function_check_update_name_portfolio == true) {
		        	echo "same-title";
		        }
		        else {
		        	//Set timezone
			        ...
			        $function_update_portfolio = $class_portfolio->update_portfolio($title, $desc, $photo, $photourlnc, $slider_image, $cat, $status, $metakey, $metadesc, $admin_login_id, $now_date, $portfolio_idh);
			        if($function_update_portfolio == true) 
				    	echo 'true';
				    else 
				    	echo 'false';
			    }
