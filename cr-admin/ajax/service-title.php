<?php
	...
	$value           = $_POST['servicetitle'];
    $setting_name    = $_POST['settingname'];
    $setting_idh     = $_POST['settingIDh'];
    $psettingname    = 'Services in Page';
    if(!empty($_POST['servicepage'])) {
    	$servicesinpage  = implode(',', $_POST['servicepage']);
	}
    ...
    if(empty($value) || empty($setting_name) || empty($setting_idh)) {
	    ...
	    if(!empty($value) && strlen($value)>50) {
			...
			if(!empty($_POST['servicepage'])) {
                ...
                $set_services_page = $class_settings->update_settings($servicesinpage, $psettingname, $admin_login_id, $function_services_pageid, $now_date);  
            }
