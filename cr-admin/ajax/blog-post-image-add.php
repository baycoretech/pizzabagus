<?php
    ...
    if(empty($title) || empty($content) || empty($cat) || empty($status)) {
    	...
    	if($photo == MURL."cr-include/images/default-post-image.png") {
			echo "image-empty";
		}
		else {
			if(strlen($title) > 200) {
		    ...
		    elseif(strlen($metadesc) > 155) {
		        ...
		        if($function_check_blog_name == true) {
			        ...
			        $function_add_blog_image = $class_blog->add_blog_image($title, $content, $blog_type_id, $tags, $noimplodetags, $photo, $cat, $status, $comment, $metakey, $metadesc, $admin_login_id, $now_date);
			        if($function_add_blog_image == true) 
				    	echo 'true';
				    else 
				    	echo 'false';
			    }
