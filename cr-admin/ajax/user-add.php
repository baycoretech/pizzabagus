<?php
	...
	elseif(strlen($displayname) > 20) {
		...
		elseif(!empty($tw) && filter_var($tw, FILTER_VALIDATE_URL) === false) {
	    	...
	    	$function_add_administrator = $class_administrator->add_administrator($username, $password, $email, $photo, $displayname, $level, $aboutyou, $fb, $gp, $tw, $admin_login_id, $now_date);
	    	if($function_add_administrator == true) {
    			echo 'true';
	    	}
	    	elseif($function_add_administrator == 'same-name') {
	    		echo 'same-name';
	    	}
	    	else 
	    		echo 'false';
