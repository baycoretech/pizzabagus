<?php
    ...
    if(empty($name) || empty($name_id) || empty($price) || empty($size)){
        ...
        elseif(!empty($name_id) && strlen($name_id) < 3) {
            ...
            $function_add_additional_toppings = $class_additional_toppings->add_additional_toppings($name, $name_id, $price, $menu_category, $size, $admin_login_id, $now_date);
            if($function_add_additional_toppings == true) {
