<?php
    ...
    if(empty($name) || empty($name_id)){
	    ...
	    elseif(!empty($name_id) && strlen($name_id) < 3) {
	    	...
	    	$check_name = $class_ourmenu_category->check_name_ourmenu_category($name);
	    	if($check_name == true) {
	    		echo "same-name";
	    	}
	    	else {
	            ...
	            $function_add_ourmenu_category = $class_ourmenu_category->add_ourmenu_category($name, $name_id, $page_link, $admin_login_id, $now_date);
	            if($function_add_ourmenu_category == true) {
