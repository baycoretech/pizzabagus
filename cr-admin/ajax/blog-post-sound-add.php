<?php
    ...
    if(empty($title) || empty($content) || empty($cat) || empty($status)) {
	    ...
	    elseif(strpos($link, 'soundcloud') === false || filter_var($link, FILTER_VALIDATE_URL) === false) {
			echo "no-soundcloud";
	    }
	    ...
	    elseif(strlen($metadesc) > 155) {
			...
			$function_check_blog_name = $class_blog->check_blog_name($title);
	        if($function_check_blog_name == true) {
	        	echo "same-title";
	        }
	        else {
	        	//Set timezone
		        ...
		        $function_add_blog_sound = $class_blog->add_blog_sound($title, $content, $link, $blog_type_id, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $admin_login_id, $now_date);
		        if($function_add_blog_sound == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
		    }
