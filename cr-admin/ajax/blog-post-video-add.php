<?php
    ...
    if(empty($title) || empty($content) || empty($cat) || empty($status)) {
	    ...
	    elseif(strpos($link, 'youtube') === false || filter_var($link, FILTER_VALIDATE_URL) === false) {
			echo "no-youtube";
	    }
	    ...
	    elseif(strlen($metadesc) > 155) {
	        ...
	        if($function_check_blog_name == true) {
		        ...
		        $function_add_blog_video = $class_blog->add_blog_video($title, $content, $link, $blog_type_id, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $admin_login_id, $now_date);
		        if($function_add_blog_video == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
		    }
