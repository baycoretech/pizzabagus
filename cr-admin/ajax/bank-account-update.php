    ...
    if(empty($name) || empty($number) || empty($owner)) {
	    ...
	    if(empty($image)) {
			...
			elseif(!empty($owner) && strlen($owner)>255) {
		    	...
		    	$function_update_bank_account = $class_bank->update_bank_account($name, $number, $owner, $image, $admin_login_id, $now_date, $bank_idh);
		    	if($function_update_bank_account == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
		    }
