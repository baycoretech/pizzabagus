    ...
    if(empty($title) || empty($content) || empty($cat) || empty($status)) {
	    ...
	    elseif(strlen($metadesc) > 155) {
	        ...
	        if($function_check_blog_name == true) {
		        ...
		        $function_add_blog_standard = $class_blog->add_blog_standard($title, $content, $blog_type_id, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $admin_login_id, $now_date);
		        if($function_add_blog_standard == true) 
			    	echo 'true';
			    else 
			    	echo 'false';
		    }
