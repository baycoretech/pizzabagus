<?php
	...
	elseif(strlen($metadesc)>155) {
		...
		if($v_checkBlogTitle==0) {
    		$v_getUpdateSoundPost = $o_getBlog->updateBlogSound($title, $content, $link, $blogtype, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $blogIDh, $adminLoginID);
    		echo "success";
    	}
    	else {
    		echo "same-title";
