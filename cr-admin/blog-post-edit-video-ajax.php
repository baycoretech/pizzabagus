<?php
	...
	elseif(strlen($metadesc)>155) {
		...
		if($v_checkBlogTitle==0) {
    		$v_getUpdateVideoPost = $o_getBlog->updateBlogVideo($title, $content, $link, $blogtype, $tags, $noimplodetags, $cat, $status, $comment, $metakey, $metadesc, $blogIDh, $adminLoginID);
    		echo "success";
    	}
    	else {
    		echo "same-title";
