<?php
    ...
    $blogcategoryIDh = $_POST['blogcategoryIDh'];
    $adminLoginID    = $_POST['adminLoginID'];

    if(empty($name) || empty($blogcategoryIDh) || empty($adminLoginID)){
        echo "field-empty";           
    }
    ...
    elseif($name=="page" || $name=="cat" || $name=="tag") {
    	echo "reserved-word";
    }
    else {
        ...
        if($v_checkBCName==1) {
        	...
        	$v_getUpdateBC = $o_getBC->updateBlogcategory($name, $nameold, $blogcategoryIDh, $adminLoginID);
